...
>
> 📍 현재 문서: **04-architecture-maturity** (4/17)
>
> **권장 읽기 순서**: 01 → 06 → 03 → **04** → 05 → 02 → 10 → 07 → 08 → 09 → 14 → 15 → 12 → 11 → 13 → 16 → 17
>
> 이전: [03-ai-collaboration-patterns](./03-ai-collaboration-patterns.md) · 다음: [05-core-code-review](./05-core-code-review.md)
...
- [ ] feature/ 디렉토리 간 import 의존성 분석 (교차 참조 여부)
- [ ] Zod 스키마와 TypeScript 타입 간 동기화 정확도 검증
- [ ] generateStaticParams()의 빌드 타임 성능 측정 (21개 vs 100개 노드)
- [ ] dynamic import의 실제 코드 스플리팅 효과 번들 분석

### 관련 소스 파일
- `src/features/` — 피처별 코로케이션 구조 전체
- `src/features/content/types/schema.ts` — Zod 스키마 정의
- `src/app/nodes/[slug]/page.tsx` — SSG + 메타데이터 생성
- `tsconfig.json` — TypeScript strict 설정

### 관련 회고 문서
- [06-decision-timeline](./06-decision-timeline.md) — 아키텍처 결정의 "왜"
- [05-core-code-review](./05-core-code-review.md) — 실제 구현 코드 리뷰
- [07-testing-strategy](./07-testing-strategy.md) — 아키텍처 검증 수준
