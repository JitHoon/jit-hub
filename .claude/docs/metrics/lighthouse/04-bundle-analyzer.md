...
**문제**: 홈 페이지가 `searchParams` 사용으로 Dynamic 렌더링(`ƒ /`) 중.
매 요청마다 서버 렌더링 → TTFB 증가 → FCP/LCP/SI 악화.

**해결**: `page.tsx`에서 `searchParams` 의존 제거 → `useSearchParams()` 클라이언트 처리로 전환.
콘텐츠 로딩은 server action(`serialize`) + 클라이언트 `MDXRemote`로 변경.

**변경 결과**: Route 테이블에서 `ƒ /` → `○ /` (Static) 전환 확인.

**영향 지표**: FCP (10%), LCP (25%), SI (10%) — 합산 가중치 45%

**수정 파일**:

| 파일 | 변경 |
|------|------|
| `src/app/page.tsx` | `searchParams`/`generateMetadata` 제거, 정적 서버 컴포넌트로 단순화 |
| `src/app/HomeLayout.tsx` | `useSearchParams()` + `Suspense` 래퍼 추가, `ClientContentSection` 사용 |
| `src/features/content/actions/getSerializedContent.ts` | 신규: server action, MDX serialize |
| `src/features/content/components/ClientContentSection.tsx` | 신규: 클라이언트 MDX 렌더러 + 스켈레톤 |

#### 2. HomeLayout 서버/클라이언트 컴포넌트 분리

**문제**: `HomeLayout` 전체가 `"use client"`.
헤더 제목, 푸터, 트리 목록 등 정적 UI까지 클라이언트 번들에 포함.

**해결**: 정적 UI를 서버 컴포넌트로 분리, 인터랙티브 부분(GraphSection, NodeSearch)만 클라이언트로 유지.

**영향 지표**: TBT (30%), FCP (10%)

**상태**: 미착수. 컴포넌트 간 상태 공유(hoveredNode, selectedNode)가 많아 분리 난이도 높음.

### 기대 효과

| 지표 | 개선 전 | 정적 셸 후 (예상) | 가중치 |
|------|---------|-----------------|--------|
| FCP | 1.6s | ~1.1s | 10% |
| LCP | 1.9s | ~1.4s | 25% |
| SI | 3.9s | ~2.5s | 10% |
| TBT | 400ms | 400ms (변화 없음) | 30% |
| CLS | 0 | 0 | 25% |

**예상 점수: 88 → 92~95점** (three.js TBT 400ms는 구조적 한계로 100점 불가)
배포 후 Lighthouse 실측으로 확인 필요.
...
| 순위 | 작업 | 예상 효과 | 난이도 |
|------|------|----------|--------|
| 1 | 배포 후 Lighthouse 실측 | 정적 셸 효과 수치 확인 | XS |
| 2 | HomeLayout 서버/클라이언트 분리 | TBT/FCP 개선 | M |
| 3 | CSS 미사용 규칙 정리 | 26KB 절감 | XS |
