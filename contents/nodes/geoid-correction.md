...
- **← 좌표계 변환** (수평 좌표 변환 후, 수직 좌표 보정이 필요)
- **← Mesh·BIM·Point Cloud** (BIM 특유의 좌표 불일치 문제)
- **→ modelMatrix 위치·회전·스케일** (보정된 높이가 modelMatrix에 반영)
- **→ IFC→3D Tiles 파이프라인** (tileset.json 생성 시 높이 보정 적용)
