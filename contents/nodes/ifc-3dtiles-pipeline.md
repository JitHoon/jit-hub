...
- **← 3D Tiles 스펙** (이 파이프라인이 생성하는 포맷)
- **← Mesh·BIM·Point Cloud** (BIM/IFC가 입력 데이터)
- **← 좌표계 변환** (tileset.json 좌표 정합에 적용)
- **← 지오이드고 보정** (높이 보정에 적용)
- **← 자오선 수차 보정** (방향 보정에 적용)
- **→ 공간 분할 기법** (타일링에 사용하는 기법)
- **→ LOD 레벨 오브 디테일** (정밀도 단계 생성)
- **→ Draco 지오메트리 압축** (최종 타일 크기 절감)
