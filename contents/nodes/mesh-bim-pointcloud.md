...
- **← Cesium.js 도입 결정** (Mesh 시각화를 가능하게 한 의사결정)
- **← 3D Tiles 스펙** (이 데이터를 웹에서 시각화하는 포맷)
- **→ IFC→3D Tiles 파이프라인** (BIM 데이터의 변환 시스템)
- **→ 지오이드고 보정** (BIM 특유의 좌표 문제)
- **→ 측정 도구 7종** (Point Cloud + Clipping Plane 단면도)
