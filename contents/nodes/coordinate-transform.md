...
- **← Cesium.js 도입 결정** (PoC에서 발견된 문제)
- **← 3D Tiles 스펙** (boundingVolume의 region이 요구하는 좌표 체계)
- **→ 지오이드고 보정** (높이 좌표의 추가 보정 — 정표고 vs 타원체고 불일치)
- **→ 자오선 수차 보정** (방향의 추가 보정 — Grid North vs True North 회전 오차)
- **→ modelMatrix 위치·회전·스케일** (변환된 좌표로 모델 배치)
- **→ IFC→3D Tiles 파이프라인** (tileset.json 생성 시 좌표 정합에 적용)
